...
- [x] **Task 6: acceptance and records.** Owns `apps/desktop/e2e/`, the visual baselines, `cspell-words.txt`, and `docs/adr/`. Depends on tasks 2 through 5, because it drives the app they assemble.
  ...
  - [x] `cspell-words.txt` gains the vocabulary the diff introduces: the runtime and vendor slugs plus the change documents' own words, verified with the gate run outside the worktree blind spot. The adversarial review caught the words the first sweep missed.
