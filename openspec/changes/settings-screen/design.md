...
- **A save names only the fields it changes.** `settings:save` used to carry the whole document. That left the main process unable to tell a person moving the switch from a renderer cache repeating a value it read earlier. Neither gate direction was correct while that held. Comparing against the stored value killed the switch, and comparing against the machine let an unrelated save rewrite the login item. The channel now carries a patch, `withSettingsPatch` merges it into the stored document in one place, and the login item takes a write only when the patch names it. React Query never revalidates on an app switch here, because an Electron window that loses focus while staying on screen fires no `visibilitychange`. A stale cache was therefore the ordinary case rather than a race.
...
The second review round reproduced two more. The failure surface offered a "Try again" that cleared the error boundary without re-running the loader, so it never retried anything. It also printed the raw Node message with the account name in it. The button now invalidates the router, proven by a test that repairs the loader and watches the page come back. `withoutHome` shortens the home directory wherever a failure message carries it. The acceptance step for the inert switch reads the document on disk rather than through `settings:get`, which answers the operating system reading and can no longer fail on an unpackaged launch.
