...
- [x] **Task 9: port the reference test suite and the reasoning mapping.** Owns the codec test and source files it extends (`chat-completions-codec.*`, `responses-codec.*`, their testkits and drops). Depends on tasks 5, 6, and 7, because it strengthens the codecs they land. Added on 2026-08-05 after the maintainer read the CLIProxyAPI reference tests.
  - [x] Port the reference request cases as fixtures and drive any missing behavior: tool-call id sanitization consistent across `tool_use` and `tool_result`, parallel tool-result grouping into one user turn, base64 and URL image blocks in tool results, system and developer roles collapsing to ordered system blocks, the system-only fallback user message, cache-control preservation with part-over-message and last-block precedence, and root schema union normalization. Cache-control, developer-role collapse, and dual base64-plus-URL tool-result images live on the Chat Completions leg; the Responses input carries only user and assistant roles, so its leg lands sanitization, grouping, the fallback user, the root union, and a base64 `function_call_output` image.
  - [x] Port the reference response cases: the usage mapping where prompt tokens sum input and cache-read and cache-creation, cache-read maps to cached tokens, and a `message_start` usage merges with a later `message_delta` where output tokens overwrite rather than sum.
  - [x] Implement the reasoning mapping the amended decision 4 names: a Responses reasoning item crosses to an Anthropic thinking block on a compatible signature, to a redacted thinking block on redacted content, and drops on a foreign-provider signature. The `previous_response_id` handle stays a typed refusal. The hub gained a `redacted_thinking` content block so the Anthropic-shaped pivot can carry it (ADR-0075 follow-up).
  - [x] Every ported case reads as a Given/When/Then behavior spec, so the suite proves recompose passes the same cases the reference does.
  - [x] Layers: unit and property, with the ported fixtures.
  - [x] An adversarial review round hardened the codecs and settled the review-driven posture in ADR-0076: a message-less conversation and a colliding sanitized tool id refuse typed rather than fabricate a turn or mispair, adjacent same-role turns merge for an alternating target, the stream decoder skips an unknown provider output item rather than throwing, hub `inputTokens` excludes cache reads, and a dropped tool-result image records a cost-bearing fate.
