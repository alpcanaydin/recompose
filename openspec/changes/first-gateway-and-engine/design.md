...
**Decision.** `packages/engine` serves each gateway through a Hono app and opens sockets through `@hono/node-server`. Hono's `app.request()` runs every routing, guard, and refusal behavior against a real Request with no socket, so only the listener module ever binds one. The adapter returns a real Node server, keeping `close()`, `closeAllConnections()`, and the `error` event. Both packages carry a license on the repository's allowlist, and Hono ships zero runtime dependencies. Throughput isn't a reason, and benchmarks appear nowhere in this record. Nothing enforces a token this round, as a named deferral. The settings screen no longer carries the switch, and the vault still holds the token, so the gateway settings feature wires the two together.
...
- **Task 4: the main process, and the five engine channels.** (depends on: task 2, because the fork imports the child entry it produces, and on task 3 for `apps/desktop/vitest.config.ts`, a shared-file blocker resolved by task 3 landing first) Owns `apps/desktop/src/main/` outside the settings files task 1 touched, both preload files, the resources, `apps/desktop/package.json`, and `apps/desktop/stryker.config.json`. It also owns the five channel entries in `packages/contracts/src/ipc.ts`, which a replan moved here from task 1.

  **Replan, approved after task 1.** `IpcHandlers` in `apps/desktop/src/main/ipc/dispatch.ts` is a total mapped type over `IpcChannel`. Growing `ipcChannels` breaks the desktop typecheck until every new channel carries a real handler, and three of the five need the engine host, which needs the engine package. Task 1 refused both escapes on principle. A handler body returning a failure it never met would be a lie the clean-code rules forbid. Relaxing `IpcHandlers` to `Partial` would trade away the compile-time totality ADR-0018 bought. So task 1 landed `ipcEvents` and its derived types, which break nothing, and parked `engine:start`, `engine:stop`, `engine:states`, `gateways:offer-port`, and `gateways:move-port`. They land with task 4 in the same commit as `engine-ipc.ts`, the `ipcChannelNames` entries, and the five preload entries. **Task 5 reads a twelve-channel bridge until task 4 lands.**
