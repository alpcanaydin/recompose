...
- [ ] **Task 7b: the live model-list lane.** Owns the model-list directive and report arms in `packages/contracts/src/engine-protocol.ts`, the fetch in `packages/engine/src/engine-child.ts` beside the probe arm, the main handler and its `ipcChannels` entry, and the wiring in `register-ipc.ts`. Depends on tasks 4 and 5, because it edits the same engine and main files. Runs alone.
