...
- **Every new component under `ui/` ships its story in the same commit as the component.** A cluster that adds a component without one gets stopped at `pre-push` by `pnpm run lint:stories`, which names the file. Leaving it for later costs a whole review round: the gap surfaced once at the pull request, after ninety commits, when seven components had gone storyless and the design sweep that walked every story had walked past all seven.
