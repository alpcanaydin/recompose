...
The phases name enforcement machinery. The finding-by-commit verifiers stay deferred until a feature has run through the pipeline, because building them before a round has produced findings would fix their shape against guesswork. Until they exist, the session confirms a fix by hand. The rider ledger is the repository's issue tracker under the `rider` label, and [verification.md](references/verification.md) carries the convention. The review workflow, the process assertion, the citation validator, and the edit-time test-first gate now exist. The `review-pr` saved workflow at `.claude/workflows/review-pr.js` carries the first two, and it runs on the local head before a pull request exists rather than behind a continuous integration check (ADR-0051). The `feature-kickoff` saved workflow at `.claude/workflows/feature-kickoff.js` dispatches discovery and drives the citation validator, which lives at `.claude/workflows/citation-validator/citation-validator.mts`. The gate is four pieces: `@nizos/probity` pinned exact as a root development dependency, a `PreToolUse` hook on the editing tools, the resolver that hook runs at `.claude/workflows/hooks/resolve-transcript.mts`, and `probity.config.ts` at the repository root for the scope.
