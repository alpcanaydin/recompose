...
The phases name enforcement machinery. The citation validator, the environment setup script, the finding-by-commit verifiers, and the rider ledger stay deferred to later changes. Until each one exists, the session runs those checks by hand and records riders in the pull request body. The review workflow, the process assertion, the pipeline marker, the path guard, and the edit-time test-first gate now exist. The `review-pr` saved workflow at `.claude/workflows/review-pr.js` carries the first three, and the path guard lives at `.claude/workflows/path-guard/`. The gate is four pieces: `@nizos/probity` pinned exact as a root development dependency, a `PreToolUse` hook on the editing tools, the resolver that hook runs at `.claude/workflows/hooks/resolve-transcript.mts`, and `probity.config.ts` at the repository root for the scope.
