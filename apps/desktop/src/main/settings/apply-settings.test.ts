...
describe('what a settings document changes outside the window', () => {
  test('the theme the document names is the theme the operating system paints', () => {
    ...
    applyChosenSettings(effects, { ...defaultSettings(), theme: 'dark' }, undefined);
  ...
  test('the menu bar carries an icon while the document asks for one', () => {
    ...
    applyChosenSettings(effects, { ...defaultSettings(), showInMenuBar: true }, undefined);

    expect(applied.menuBarVisible).toBe(true);
  });

  test('the operating system launches recompose when the save asks for it', () => {
    const { effects, applied } = recordingEffects();

    applyChosenSettings(effects, { ...defaultSettings(), launchAtLogin: true }, true);

    expect(applied.loginItem).toBe(true);
  });

  test('a save that never names the login item leaves the operating system alone', () => {
    const { effects, applied } = recordingEffects();

    applyChosenSettings(effects, { ...defaultSettings(), launchAtLogin: true }, undefined);

    expect(applied.loginItem).toBeNull();
  });

  test('a save that asks for nothing turns all three back off', () => {
    const { effects, applied } = recordingEffects();
  ...
  propertyTest.prop([anySettings])('every chosen document reaches all three effects', (fields) => {
    ...
    applyChosenSettings(effects, { ...defaultSettings(), ...fields }, fields.launchAtLogin);
...
describe('the login item a save does not touch', () => {
  test('an unrelated change leaves the operating system holding what it holds', () => {
    ...
    applyChosenSettings(effects, { ...stored, theme: 'dark' }, undefined);
