...
- **The cap lives in the workflow rather than in the validator.** The cap governs dispatch, and the dispatching machinery is the workflow. `assertArmCapRespected` throws when the folded arm table holds more than six arms, and it runs before the parallel dispatch. The `full` tier folds to four arms and the `standard` tier to two, so the assertion has headroom today. A later contributor who adds a seventh arm meets a throw rather than a silent breach.

- **A dead code-map arm throws, and a dead research arm logs.** The dispatch hook answers `null` on a terminal failure. `resolveArm` keys the throw on the arm's subagent type, so a `code-analyzer` arm that died, or one that failed to write its file, stops the run. The validation phase would otherwise have nothing to check. A `researcher` arm logs and the run continues, because its brief feeds a human step rather than a machine gate. One consequence of keying on the type: the `rider-ledger` arm on the `full` tier produces a brief while riding the `code-analyzer` type, so its death throws too. That's stricter than the reason behind the rule requires.

## Limits this design keeps

A future reader who trusts the mechanism further than these four bounds allow will rediscover them the expensive way, so this section states them plainly.
...
**Good**: discovery runs from a saved workflow, so the arms and the cap no longer depend on the orchestrating session improvising from a prose table. A fabricated path or an invented symbol fails at no model cost, before the solution design consumes it, and the reader gets one repair attempt with the failures named. Each arm writes its own file into `openspec/changes/<slug>/discovery/`, so a later phase in a fresh session reads the findings from disk. The verdict function stays pure and takes its file reader as an argument, so its unit spec needs no filesystem.
