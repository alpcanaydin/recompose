...
Every technical decision in recompose is recorded here (see `CLAUDE.md`). New ADRs: copy the lightweight format of an existing one, number sequentially, add a row below. Never edit an accepted ADR — supersede it with a new one and update its status.
...
| [0019](0019-turbo-cache-persistence-in-ci.md)                  | Turbo Cache Persistence in CI via actions/cache                  | Accepted | 2026-07-23 |
