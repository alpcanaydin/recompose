...
- **Turbo wiring**: per-package `test` script (Turborepo guidance over root-level projects), `coverage/**` cached as task output. CI gains one step: `playwright install --with-deps chromium`. The root package.json carries a pinned vitest devDependency solely so vitest.shared.ts can type its export against vitest/node.
