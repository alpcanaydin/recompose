...
- **The app presents itself as Recompose.** `productName`, `app.setName`, the About panel, the application-menu label, and the tray tooltip all move to title case. Same-file ownership of `electron-builder.yml` is the project's named reason to serialize, so the rename rides this change. The bundle identifier stays `sh.recompose.app`, and prose and package names stay lowercase.
- **The feature files stay the review contract.** The four scenarios files remain the approval artifact. Scenarios a machine can judge land as assertions in `packaged-smoke.spec.ts`, titled after the scenarios they prove. The glass edge and the tray tint go to the maintainer's manual pass, because no process observes them.
