...
- **One shared subagent style without pins**: rejected. A model-diverse reviewer pair and judges with no write tools need explicit `model` and `tools` keys. A single default erases that control.
