...
- **The subagent roster follows one convention.** Six definitions under `.claude/agents/` share a shape. Each carries a trigger-rule `description` that states when the subagent fires and an explicit `model` pin. `skills:` preloads appear where a skill replaces duplicated instructions, not on every definition. Judges take a `tools` list with no edit tools, reviewers take `memory: project`, and the implementer takes `isolation: worktree`. The subagent configuration documents the `skills`, `memory`, and `isolation` keys as supported fields. One caveat holds: the loader skips `memory` with no warning when the harness disables project memory.
...
- **Change hygiene means real deltas and no `--skip-specs`.** Every change carries at least one spec delta with a scenario from its first commit, because the validate gate fails a change that holds none. Meta changes write their deltas into `development-process`. The `--skip-specs` flag on `openspec archive` sets the `skipSpecs` option, which acts at archive time alone. It never bypasses validation, and it skips the fold of deltas into `openspec/specs/`. This change once carried an inert `skip_specs: true` line that implied an escape the tool never reads. The change-metadata schema drops unknown keys, so the line changed nothing. A later commit removed it as dead configuration. The fold skips only when someone passes `--skip-specs` to `openspec archive`. The policy holds: changes carry real deltas, and no one passes `--skip-specs` for a change that has them.
...
- **One shared subagent style without pins**: rejected. A model-diverse reviewer pair and judges with no edit tools need explicit `model` and `tools` keys. A single default erases that control.
- **Passing `--skip-specs` for a change with deltas**: rejected. It skips the fold at archive and buys nothing at validation. Real deltas from the first commit keep the gate green instead.
...
**Bad, and accepted**: the skill restates process knowledge that drifts as the pipeline evolves. The planned instruction-drift audit owns that risk in a later change. Linted subagent files add writing friction, accepted once because the definitions stay stable. The template adds weight on small features, which the standard tier trims. Policy bans the `--skip-specs` flag, and nothing machine-checks the ban.
