# 0010: Folder structure, feature-sliced renderer, enforced boundaries
...
this file go") must leave no room for interpretation, so they can't drift
...
- **Monorepo map**: `apps/desktop` (exists) plus reserved names
  ...
  `contracts`; engine internals get their own decision record when the package opens.
- **Main process**: `main/index.ts` is bootstrap only; modules
  `windows/`, `ipc/`, `engine-host/` (utilityProcess spawn + engine
  entry). `preload/` stays a single file by design, because growth there
  means the exposed API surface is too wide.
- **Renderer**: Feature-Sliced Design (FSD) v2.1: layers
...
- **Enforcement**: Steiger for FSD rules, dependency-cruiser for package
  direction, process boundaries, engine purity, and import cycles, though
  the upcoming tooling jobs configure them; this decision fixes the rules
  now.

## Alternatives

- **Bespoke feature-based hybrid (bulletproof-react style)**: lighter,
  but its rules exist only as far as the team writes them, so every
  unwritten gap becomes interpretation room, exactly what this decision
  removes. FSD is externally documented and has an official linter.
- **Technical layers (`components/hooks/utils`)**: doesn't scale past a
...
- **Steiger alone (no dependency-cruiser)**: Steiger only sees FSD; it
  can't express monorepo direction, Electron process borders, or the
  engine purity rule.
...
on a one-person project. Two structure linters need configuring and
keeping green.
