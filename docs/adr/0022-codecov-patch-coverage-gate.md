# ADR-0022: Patch Coverage Gate via Codecov
...
The project coverage gate (90% thresholds, ratcheting per ADR-0015) judges the whole tree, so a PR can add poorly covered lines while global numbers stay green — dilution hides regressions at the margin. The control-gates queue orders a diff-scoped gate third: changed lines in a PR must meet a high threshold while the project gate stays untouched. The queue originally demanded a self-contained tool; the maintainer relaxed that to allow free SaaS, which changes the answer.
...
- **covguard / cover-diff** — diff-scoped npm-ecosystem tools, but neither is published on npm; GitHub-only installs fail the supply-chain bar (pinned, registry-quarantined dependencies per ADR-0015).
