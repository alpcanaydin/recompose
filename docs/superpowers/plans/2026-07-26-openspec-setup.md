...
- Create: `.claude/skills/opsx-*` (via CLI, exact names depend on the installed version)
- Modify: `skills-lock.json` (record the vendored opsx skills)
- Possibly modify: `CLAUDE.md` or `AGENTS.md` (init may inject a pointer block — inspect the diff)
...
- [ ] **Step 5: Record the vendored skills in the lock file**

Open `skills-lock.json`, copy the entry shape used by existing vendored skills (see the `playwright-best-practices` entry), and add one entry per new `opsx-*` skill directory.
