...
**Execution deviation:** the shipped configuration grew past this table to 17 rules, including the transitive `reachable` checks. The authoritative inventory is `.dependency-cruiser.cjs` itself. ADR-0014 records the decision and its strictness posture.
