...
- **A supply chain gate runs before the draft exists.** Syft generates a CycloneDX Software Bill of Materials (SBOM) per platform leg, `actions/attest-sbom` and `actions/attest-build-provenance` attach attestations (`id-token: write`, `attestations: write`), and a license gate walks the production dependency tree through `pnpm licenses list --prod --json` against a committed allowlist (`MIT`, `Apache-2.0`, `BSD` variants, `ISC`, and peers). A license outside the list fails the release; the gate blocks, never advises.
