...
- **The acceptance suite covers the providers flow and the home screen.** `home.feature`: a fresh install shows the empty state. `providers.feature`: connecting an account lists it, and removing an account clears it. Real app, real IPC, real storage in the temporary directory. On Linux the vault requires an operating-system keyring, so the continuous integration leg provides an unlocked gnome-keyring inside a dbus session. A keyring-less Linux host refuses account connects by design.
...
- **The browser-cache rider lands on the existing `check` job.** The Chromium download for the Vitest browser project gets cached on `~/.cache/ms-playwright`, keyed on the pnpm lockfile's hash.
