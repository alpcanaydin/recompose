# recompose: design document

**Date:** 2026-07-21. First version 2026-07-20, renamed from "rewire" and reconciled with the design system on 2026-07-21.
...
- **OSS**, no commercial intent.
- **UI-first**: the visual/UX is fully designed first, and the backend wiring follows afterward.
...
- Character: **DESKTOP-native, macOS-first, "Mac Native" flat pro tool** (TablePlus/Xcode flavor): traffic lights top-left, flat dark/light chrome, systemBlue accent, SF feel. Not a web app.
  ...
  - On the native side this corresponds to `NSVisualEffectView` sidebar vibrancy, with no heavy CSS glass.
- **The window fills the viewport:** `.win` has a symmetric 28px margin, `.app` height is `calc(100vh - 58px)`, `min-height: 640px`. Browser fullscreen is a faithful app simulation. The canvas absorbs extra height, and the minimap stays pinned bottom-right.
- Signature: **live green flowing dots** on the wires (real-time traffic). The status language extends to the sidebar dots.

---

## 3. Information architecture and app shell
...
Apple Human Interface Guidelines (HIG) source-list language (28px rows / 13px text / 16px symbols, no search). Taxonomy (Mobbin: Cursor, Relevance AI):

- **Local Gateways** (renamed from "Virtual Gateways"): rows are `name` + live status dot (🟢 pulse = traffic · ⚪️ = idle · 🟡 = rate-limit/failover · 🔴 = stopped/error).
- **Providers**, with children **Subscriptions / API Keys / Aggregators** + count badges (renamed from "Accounts").
...
- **Center:** the **address-bar pill** is a small teal `net` glyph plus the gateway address in mono, `http://localhost:8397/my-gateway · running` (flex: 1). No brand mark in the pill (see §3.1). Reference: Firecrawl.
...
This design eliminated the Canvas/Logs/Config tabs. The log panel opens from the toolbar panel toggle. Reference: VS Code, GitHub Codespaces, Replit, Weavy.
...
- **Bottom log panel:** toggled by the toolbar `panel-b` button, full-width, 190px tall, scoped to the current selection. The canvas pans up (−72px) while it's open. See §7.
...
- A gateway serves **multiple Virtual Models**: the canonical screen shows **3 VMs** (`creative`, `fast`, `code`) fanning out from the Gateway node. Reference: Writer (closest), OpenAI AgentKit, Runway.
...
- **Color coding (full-tint node borders):** Gateway **teal** `#40c8e0` · Virtual Model blue `#0a84ff` · Router orange `#ff9f0a` · Target purple `#bf5af2`. (This direction change eliminated the old gray gateway.) Other tokens: green `#32d74b`, red `#ff453a`, yellow `#ffd60a`. Light-theme equivalents live in the DS tokens (`#007aff`, `#28cd41`, teal light `#30b0c7`…).

### 4.2 Building nodes / wiring

- **Primary:** drag from a port → on release, a "what should this connect to?" menu (n8n logic).
...
The graph keeps a fixed layer order (Gateway·VM·[Router]·Target). Crowding comes from parallel VM count.

- **Baseline = free drag** (manual positioning).
- **Zoom cluster (− 100% + expand) bottom-left of the canvas**, **minimap bottom-right** (this design tried moving zoom into the status bar, then reverted it).
...
### 5.1 Gateway node → **Overview / Connect** (two tabs only, this design removed the Logs tab on 2026-07-21). Logs live in the bottom panel, see §7. Template: `templates/gateway-detail/`.

- **Overview = aggregate NUMBERS** (the canvas already shows live routing health, so the target list isn't repeated):
  - **Endpoint** box: Base URL `http://localhost:8397/my-gateway` + Status `Running · 3d 4h`.
  - **Last 24h** 2×2 Key Performance Indicator (KPI) grid: `12.4k requests · 8.2M tokens · 0.3% errors · 1.1s p95`.
  - **Serves · 3 models**: share bars per VM, creative 54% (blue) / fast 31% (green) / code 15% (purple).
  - Decision: **A KPI cards** won, and B (routing-health list) lost. Reference: Cloudflare Workers, WRITER Observability, Laravel Cloud.
...
**NO** separate top-level Logs page exists, and **NO Logs tab exists in the inspector**: "the bottom bar shows the selection's logs." The log stream lives in a **full-width bottom panel** (190px), toggled by the toolbar `panel-b` button, **scoped to the current selection** (for example, `Logs · My Gateway`). Template: `templates/gateway-detail/` screen 4.
...
- The dock form (B) won over the old drawer-tab option (A). Reference (Mobbin): Stripe Logs, Supabase, Base44, Vercel Live tail.
...
- **Destructive actions ask for confirmation** (modal: "connected clients will lose this endpoint · can't be undone").
- Reference: Coda, Toggl, Lemni.

### 8.2 Error states, in-place plus a banner/toast (companion: `error-states.html`)
...
- **This design eliminated C (a separate health center)**: the sidebar dots + canvas status language already aggregate health.
- Reference (right direction): Better Stack, Google Meet inline instructions.

### 8.3 Empty states

- Solved in onboarding: empty canvas + **checklist + ghost chain**. Other gaps (no providers → "Connect" Call To Action (CTA), no logs → "Waiting for the first request") use the same quiet inline language.

### 8.4 Local auth and network exposure

Locked 2026-07-21, following the LM Studio model. Research-driven: Ollama's no-auth stance, its Domain Name System (DNS) rebinding vulnerability (Common Vulnerabilities and Exposures (CVE) ID CVE-2024-28224), and LM Studio's flat token toggle. Three layers:
...
The main sidebar "Settings" surface → a **single scrollable grouped page** in the content area (macOS grouped-card language, 560px column). **`⌘,`** focuses this surface, no separate window. This design eliminated C (a separate Preferences window). If content grows, promoting to A (category rail) is easy. Reference (Mobbin): Substack/Vercel row anatomy, Google Drive Settings, Linear Preferences, macOS System Settings.
...
- **Appearance:** Theme (System/Light/Dark segmented) · **Reduce wire motion** (the signature flowing-dot effect, for low-power mode).
...
**Screens/states not yet mocked** (decisions are in this document, visuals pending as natural prototype-phase work):
...
- Backend protocol translation (adapting opencodex/cliproxyapi/ccproxy), coming after the UI locks.
...
> Note: this repo isn't git (`git: false`). The "commit" step doesn't apply, so the document lives as a file.
