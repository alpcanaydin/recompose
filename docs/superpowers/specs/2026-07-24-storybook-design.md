...
- **Story discipline follows Storybook's guidance for artificial-intelligence tooling.** One concept per story, JSDoc with `@summary` on components, props, and stories, `react-docgen-typescript` for prop extraction, and anti-pattern stories tagged `!manifest` so automated tooling never learns from them. ADR-0029 later superseded the `react-docgen-typescript` choice with `react-docgen`, because the TypeScript-aware extractor crashes against the repository's `typescript@7.0.2` pin.
- **Story tests become part of the suite.** `@storybook/addon-vitest` adds a third Vitest project that reuses the existing Playwright Chromium provider. Story tests therefore run inside `pnpm test` and the required CI `check` job. The lefthook pre-commit chain stops at typecheck, so it runs no story tests. Accessibility runs inside those tests with `parameters.a11y.test` set to `error`; a story opts out only with a documented reason.
...
- **Coverage thresholds stay at their current values.** Story tests feed the existing v8 coverage, and the story files themselves stay out of coverage as specs rather than product code. If a product file's coverage dips during implementation, the fix is finishing stories, never excluding the product file.
