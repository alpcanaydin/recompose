...
- **Sender validation before anything else** (mid-flight addition): every inbound invocation verifies it originates from our own window's main frame — `event.senderFrame` is null-checked (frames can be disposed) and its origin matched against the app's own origins (packaged `file://`, dev server URL) — in the testable dispatch layer, before any schema parsing; a foreign or disposed frame never reaches a handler. Whether rejection throws or returns a typed `forbidden-sender` result is decided against current Electron guidance during implementation and recorded in ADR-0018. CSP/sandbox hardening stays queue item 7.
...
- **The preload surface is closed and enumerable**: the bridge exposes exactly the contract channels (derived from `ipcChannels`, no generic invoke passthrough, no extra members), the exposed object is frozen, and a renderer spec asserts `window.recompose`'s own keys equal the channel set exactly.
