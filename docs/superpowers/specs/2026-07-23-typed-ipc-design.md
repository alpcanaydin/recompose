...
- **Validation at the boundary, both directions.** The dispatch layer parses every inbound payload with the contract's request schema before a handler receives data (the renderer is attacker-reachable territory), and it parses every outbound result with the response schema (symmetry costs little and keeps drift impossible). Handlers never re-validate.
...
- **The preload surface stays closed and enumerable**: the bridge exposes exactly the contract channels (matching the `ipcChannels` contract through explicit per-channel enumeration, no generic invoke passthrough, no extra members; ADR-0018 records why a loop-derived bridge lost) and freezes the exposed object, and a renderer spec asserts `window.recompose`'s own keys equal the channel set exactly.
