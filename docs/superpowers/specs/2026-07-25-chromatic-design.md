...
- **The Chromatic GitHub App, with the official pull-request comment turned on.** The App posts a self-updating comment carrying visual and accessibility change counts plus a link to the published Storybook. The comment is a per-project opt-in on the project's Manage screen. The UI Review feature stays off: the maintainer works solo, and the UI Tests accept flow already covers approval.
...
- The UI Review feature and reviewer assignment.
